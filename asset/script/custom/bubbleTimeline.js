class BubbleTimelineChart {
  constructor(id) {
    ...
    this.data;
    this.duration = 500;
    this.dataPath = 'asset/data/dataset.csv';
    this.gXAxis;
    this.tooltip;
    this.legendOptionData = [{ value: 3 }, { value: 30 }, { value: 60 }];

    this.mappingObject = [
      {
        label: 'Majority-Black Districts',
        filedValue: 'Majority Black districts',
      },
      {
        label: 'Black Representatives',
        filedValue: 'Black members',
      },
      {
        label: '40-50% Black Districts',
        filedValue: '40-50% Black districts',
      },
    ];
    this.myColor = d3
      .scaleOrdinal()
  ...
  loadData() {
    ...
    d3.csv(this.dataPath).then((e) => {
      ...
      const getAllMajorityBlackDistricts = this.data.map((d) =>
        Number(d['Majority-Black Districts'])
      );
      const getAllBlackMembers = this.data.map((d) =>
        Number(d['Black members'])
      );
      const getAll4050BlackDistricts = this.data.map((d) =>
        Number(d['40-50% Black districts'])
      );
      ...
      this.cricleScale.domain([
        0,
        d3.max([
          ...getAllMajorityBlackDistricts,
          ...getAllBlackMembers,
          ...getAll4050BlackDistricts,
        ]),
      ]);
  ...
  updatePath() {
    ...
    allticks.each(function (e, i, er) {
      if (Math.floor((er.length - 3) / 2) == i) {
        ...
        selfEle.x = selfEle.getTranslation(d3.select(this).attr('transform'))[0];
